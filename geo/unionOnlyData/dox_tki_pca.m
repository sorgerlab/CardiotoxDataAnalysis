...
for row = 1:size(jointDataReduced,1)
    centeredData(row,:) = jointDataReduced(row,:)-mean(jointDataReduced(row,:));
end

for row = 1:size(jointDataReduced,1)
    normData(row,:) = centeredData(row,:)./std(jointDataReduced(row,:));
end


[day1coeff,day1score,day1latent,day1tsquared,day1explained] = pca(normData','numcomponents',2,'Rows','pairwise','Centered',0);
